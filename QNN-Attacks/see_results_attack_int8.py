...
def load_configs():
    ...
    required_cfgs = ['dataset_id', 'num_classes', 'logs_dir', 'qnn_int8']
...
def main():
    ...
    test_data_file = cfgs['logs_dir'] + '/x_test_int8.bin'
    ...
    x_test_int8 = np.fromfile(test_data_file, dtype=np.int8)
    ...
    x_test_float = dequantize(x_test_int8)

    # Step 4: Load adversarial dataset and evaluate accuracy
    for x_adv_file in sorted(glob.glob(cfgs['logs_dir'] + '/adv_int8*.bin')):
        print("##############")
        print(str(x_adv_file))
        x_adv_int8 = np.fromfile(x_adv_file, dtype=np.int8)
        x_adv_int8 = np.reshape(x_adv_int8, input_shape)

        accuracy = b.get_accuracy_quant_model(qnn_int8, x_adv_int8, y_test)
        print("Int8 -> ACC: " + str(accuracy))

        x_adv_float = dequantize(x_adv_int8)
