...
import defenses.feature_squeezing_int8_backend as ft
...
def load_configs():
    ...
    parser.add_argument("--ft_color_bit", type=int, default=8, help="color bit depth")
    ...
    required_cfgs = ['dataset_id', 'qnn_source_int8', 'datasets_path']
...
def main():
    ...
    qnn_int8 = b.get_ml_quant_model(cfgs['qnn_source_int8'])

    # Step 3: Load test and adversarial datasets
    x_test_file = cfgs['datasets_path'] + 'x_test_int8.bin'
    y_test_file = cfgs['datasets_path'] + 'labels.bin'
    x_test_int8 = np.fromfile(x_test_file, dtype=np.int8)
    x_test_int8 = np.reshape(x_test_int8, input_shape)
    y_test = np.fromfile(y_test_file, dtype=np.int8)
    y_test = tf.one_hot(y_test, num_classes).numpy()

    # Step 4: Evaluate the old ANN
    accuracy_n1 = b.get_accuracy_quant_model(qnn_int8, x_test_int8, y_test)
    ...
    if cfgs['ft_calculate_threshold'] == "True":
        ...
        scaler, zp = b.get_input_quant_details(qnn_int8)
        x_train_int8 = b.quantize_dataset_int8(x_train_float, scaler, zp)
        threshold = ft.get_threshold(x_train_int8, y_train, cfgs['ft_fpr'], original_color_bit, cfgs['ft_color_bit'],
                                                        cfgs['ft_smooth_window_size'], qnn_int8, num_classes)
    else:
        threshold = cfgs['ft_threshold']

    for x_adv_file in sorted(glob.glob(cfgs['datasets_path'] + 'adv_int8*.bin')):
        print("\n" + str(x_adv_file))
        x_adv = np.fromfile(x_adv_file, dtype=np.int8)
        ...
        accuracy_n2, preds_n2 = b.get_accuracy_preds_quant_model(qnn_int8, x_adv, y_test)
        print("ADV -> Accuracy: {}%".format(accuracy_n2 * 100))
        accuracy_n3, preds_n3 = b.get_accuracy_preds_quant_model(qnn_int8, x_clean_1, y_test)
        print("SQUEEZE -> Accuracy: {}%".format(accuracy_n3 * 100))

        #preds_n2 = np.array(preds_n2, np.int8)
        #preds_n3 = np.array(preds_n3, np.int8)
#
        #x_clean_2, y_clean = ft.remove_adv_examples(x_clean_1, y_test, preds_n2, preds_n3, threshold)
        #accuracy_n4 = b.get_accuracy_quant_model(qnn_int8, x_clean_2, y_clean)
        #print("Clean: " + str(accuracy_n4))
        #
