...
def get_threshold(x, y, fpr, start_bit_depth, compression_bit_depth, spl_smooth_win_size, model, nb_classes):
    ...
    # Build the squeezers
    feature_squeezing = FeatureSqueezing_INTx(clip_values=(-128, 127), start_bit_depth=start_bit_depth,
                                                compression_bit_depth=compression_bit_depth, apply_predict=False)
    spatial_smoothing = SpatialSmoothing(window_size=spl_smooth_win_size, clip_values=(-128, 127), apply_predict=False)
    thresholds = []
    ...
    for i in range(nb_classes):
        ...
        y_before = np.array(y_before, np.int8)
        y_after_squeeze = np.array(y_after_squeeze, np.int8)
        y_after_smooth = np.array(y_after_smooth, np.int8)
