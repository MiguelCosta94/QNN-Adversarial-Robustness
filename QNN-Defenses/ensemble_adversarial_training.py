...
def load_configs():
    ...
    parser.add_argument("--ann_attack_1", type=str, default=None, help="path to the floating-point ANN to use in adversarial generation")
    parser.add_argument("--ann_attack_2", type=str, default=None, help="path to the floating-point ANN to use in adversarial generation")
    parser.add_argument("--fgsm_eps", type=float, default=0.008, help="")
    ...
    required_cfgs = ['dataset_id', 'ann_source_float', 'qnn_source_int16', 'qnn_source_int8', 'ann_new_float',
                     'qnn_new_int16', 'qnn_new_int8', 'ann_attack_1', 'ann_attack_2']
...
def main():
    ...
    # Step 4: Load models to be attacked during training and configure the attacks
    ann_attack_1 = tf.keras.models.load_model(cfgs['ann_attack_1'])
    ann_attack_2 = tf.keras.models.load_model(cfgs['ann_attack_2'])
    art_classifier_a1 = TensorFlowV2Classifier(model=ann_attack_1, clip_values=(0,1), nb_classes=num_classes,
                            input_shape=input_shape, loss_object=tf.keras.losses.CategoricalCrossentropy(), train_step=train_step)
    art_classifier_a2 = TensorFlowV2Classifier(model=ann_attack_2, clip_values=(0,1), nb_classes=num_classes,
                            input_shape=input_shape, loss_object=tf.keras.losses.CategoricalCrossentropy(), train_step=train_step)

    fgsm_1 = FastGradientMethod(estimator=art_classifier_a1, eps=cfgs['fgsm_eps'], batch_size=cfgs['batch_size'])
    fgsm_2 = FastGradientMethod(estimator=art_classifier_a2, eps=cfgs['fgsm_eps'], batch_size=cfgs['batch_size'])
    pgd_1 = ProjectedGradientDescent(estimator=art_classifier_a1, eps=cfgs['pgd_eps'], eps_step=cfgs['pgd_eps']/cfgs['pgd_max_iter'],
                            num_random_init=cfgs['pgd_num_random_init'], max_iter=cfgs['pgd_max_iter'], batch_size=cfgs['batch_size'])
    pgd_2 = ProjectedGradientDescent(estimator=art_classifier_a2, eps=cfgs['pgd_eps'], eps_step=cfgs['pgd_eps']/cfgs['pgd_max_iter'],
                            num_random_init=cfgs['pgd_num_random_init'], max_iter=cfgs['pgd_max_iter'], batch_size=cfgs['batch_size'])

    # Step 5: Perform ensemble adversarial training
    ...
    art_classifier = TensorFlowV2Classifier(model=ann_new_float, clip_values=(0,1), nb_classes=num_classes,
                        input_shape=input_shape, loss_object=tf.keras.losses.CategoricalCrossentropy(), train_step=train_step)

    adv_trainer = AdversarialTrainer(art_classifier, attacks=[fgsm_1, fgsm_2, pgd_1, pgd_2], ratio=1.0)
